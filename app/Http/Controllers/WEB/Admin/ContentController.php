...
class ContentController extends Controller
{
    ...
    public function updateSeoSetup(Request $request, $id)
    {
        ...
        if ($request->boolean('remove_meta_image')) {
            if ($page->meta_image && File::exists(public_path($page->meta_image))) {
        ...
        if ($request->hasFile('meta_image')) {
            $oldImage = $page->meta_image;
            $image = $request->file('meta_image');
            $extension = $image->getClientOriginalExtension();
            $imageName = 'seo-meta-' . $page->id . '-' . time() . '.' . $extension;
            $relativePath = 'uploads/seo/' . $imageName;

            if (!File::exists(public_path('uploads/seo'))) {
                File::makeDirectory(public_path('uploads/seo'), 0755, true);
            }

            Image::make($image)->save(public_path($relativePath));
            $page->meta_image = $relativePath;
